...
| 編號 | 卡 | 回測狀態 |
|---|---|---|
| `flows-sync-1` | 外資投信同步買超 Top30 | ＝ alpha 預註冊 **AS-01**，現成可測 |
| `flows-sync-2` | 外資投信同步賣超 Top30 | ＝ **AS-02**，現成可測 |
| `flows-oppose-1` | 外資買·投信賣 Top30 | ＝ **AS-03**（雙邊），現成可測 |
| `flows-oppose-2` | 外資賣·投信買 Top30 | ＝ **AS-04**（雙邊），現成可測 |
| `pm-aetf-1` | 主動ETF 建議句 | 無回測，且**文案需重寫**（見下） |

前四張正是 `docs/alpha-sweep-preregistration.md` 第一階段的 AS-01~04，
標「現成可測」——用現有回測快取跑一次即可，不需重抓資料。**跑出結論前不上這四張卡。**
