...
| 2 | 土洋同買 | 湧入訊號 ∩ 投信**近3日**≥2日買超 ∩ 外資**近3日**≥2日買超 | **外資當日買超金額降序**（Q1 med +1.74%） | `:197` |
| 3 | 突破新高 | **僅** 突破20日新高（單條件） | 未定，見下方待決 | `:150` 交集版陣亡 |
| 4 | 弱勢榜 | 跌破20日新低（排除跌停鎖死） | **量能趨勢降序**（分離度 0.45%） | `:82` |
| 5 | 退出＋法人賣 | 退出訊號 ∩ 法人賣強度<-5% | **不排序**（分離度僅 0.17%） | `:234` |
| 6 | 追高警示 | 爆量大漲 S≥2 R≥2% P≥0.7（**排除漲停鎖死**） | **不排序**，S 值降序呈現 | `:262` |

**卡 3 與卡 5 的偏離說明**：
- 卡 3：M2「突破∩法人買強度>5%」交集版回測陣亡（`:150`），規格退回單條件版。
  單條件版的排序欄原本未驗證 → **已決定補回測**，見 `run_sorting.py` 的 `run_m4()`。
  排序欄依 M4 結論定：候選為 法人買強度／量能趨勢／乖離率，
  採用門檻＝分離度 ≥0.30%（先訂後驗，介於已採用的卡4 0.45% 與已否決的卡5 0.17% 之間）。
  **M4 未跑出結果前，卡 3 一律不排序。**
- 卡 5：報告結論寫「採連續退出日數」（`:234`），但分離度 0.17% 實質等於無排序力。
  本規格改為**不排序**，理由是 0.17% 低於卡 4 的 0.45%，而卡 6 已因同類理由放棄排序。
  **此偏離已於 2026-07-26 由使用者確認，定案為不排序。**
...
- **regime 閘門只掛卡 1**，判定＝TAIEX 收盤 vs 自身 20MA。
- 卡 3–6 有「不翻向」的實測依據，確定不掛閘門。
- **卡 2 屬未驗證，不得外推「不翻向」**（鐵律 #4／#8）。
  → **已決定補回測**（2026-07-26），見 `run_sorting.py` 的 `run_r2()`。
  R2 跑出結果前，卡 2 一律不掛閘門且在卡面標示未驗證；R2 若判定翻向則補掛閘門。
...
- [ ] regime 閘門只作用於卡 1（測試：空頭環境下卡 1 被抑制、**卡 3–6 不變**）
- [ ] R2 已實跑，卡 2 的 regime 結論已回填本文件（不得預設「不翻向」）
- [ ] M4 已實跑，卡 3 的排序欄已依 M4 結論回填本文件（未達 0.30% 門檻則不排序）
